...
B
  ...
  // default options
  var options = {
    verificationURL: 'http://139.59.78.201:8010/email-verification/${URL}',
    URLLength: 48,
    ...
    transportOptions: {
      service: 'Gmail',
      auth: {
        user: 'abhipal1997@gmail.com',
        pass: 'password'
      }
    },
    verifyMailOptions: {
      from: 'Do Not Reply <abhipal@gmail.com>',
    ...
    confirmMailOptions: {
      from: 'Do Not Reply <abhipal1997@gmail.com>',
...
//return options; 
  ...
  var sendConfirmationEmail = function(email, callback) {
    ...
    mailOptions.to = email;
if (options.shouldSendConfirmation) { // if false, then we can't manually send confirmations either
  if (!callback) { // if callback is a falsy value, we set callback below to true; down the line, we get an error for executing a boolean
